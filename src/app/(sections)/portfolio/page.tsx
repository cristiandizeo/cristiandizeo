  ...
  return (
    ...
    <div className="w-full md:w-5/6 h-full">
      {openProjects.length === 0 && (
        <div className="flex p-4 justify-center items-center min-h-[200px]">
          <p className="text-gray-400 text-center text-lg md:text-3xl">
            ¡Hola! Podés ver mis proyectos haciendo click en ellos
          </p>
        </div>
      )}
      <div className="grid items-center grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 p-4">
        {openProjects.length > 0 &&
          openProjects.map((name) => {
            ...
            return project ? (
              <motion.div
                ...
                className="h-[28rem] bg-[#252526] border border-gray-700 rounded-lg overflow-hidden shadow-lg"
              >
                {/* Imagen del Proyecto */}
